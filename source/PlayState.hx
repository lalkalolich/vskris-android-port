	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'field-of-hopes-and-dreams':
				dialogue = CoolUtil.coolTextFile(Paths.txt('field-of-hopes-and-dreams/field-of-hopes-and-dreamsDialogue'));
			case 'rude-buster':
				dialogue = CoolUtil.coolTextFile(Paths.txt('rude-buster/rude-busterDialogue'));
			case 'the-world-revolving':
				dialogue = CoolUtil.coolTextFile(Paths.txt('the-world-revolving/the-world-revolvingDialogue'));	
		}
		...
		else if (SONG.song.toLowerCase() == 'field-of-hopes-and-dreams' || SONG.song.toLowerCase() == 'rude-buster')
			{
				defaultCamZoom = 0.9;

				curStage = 'krisstageback';
				var bg:FlxSprite = new FlxSprite(-600, 0).loadGraphic(Paths.image('krisstageback'));
				bg.antialiasing = true;
				...
				var stageFront:FlxSprite = new FlxSprite(-650, 700);
				stageFront.frames = Paths.getSparrowAtlas('krisstagefront');
				stageFront.animation.addByPrefix('idle', 'Puddle', 24, true);
		...
		else if (SONG.song.toLowerCase() == 'the-world-revolving')
				{
					defaultCamZoom = 0.9;
					curStage = 'vsjevilstage';
					var bg:FlxSprite = new FlxSprite(-600, 0);
					bg.frames = Paths.getSparrowAtlas('vsjevilstage');
					bg.animation.addByPrefix('idle', 'Symbol', 24, true);
					bg.antialiasing = true;
					bg.animation.play('idle');
					bg.scrollFactor.set(0.9, 0.9);
					bg.setGraphicSize(Std.int(bg.width * 0.9));
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				case 'vsjevilstage':
					babyArrow.frames = Paths.getSparrowAtlas('jevil/NOTE_assets');
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if(SONG.song.toLowerCase()=='the-world-revolving' || FlxG.save.data.moveNotes ){
						for(str in playerStrums){
							str.angle = 300*Math.cos((timer*2)+str.ID*2);
							str.y = strumLine.y+(-50*Math.sin((timer*2)+str.ID*2));
							str.x = str.health+(-160*Math.cos(((timer*4)+str.ID/2)));
						}
					}
					if(SONG.song.toLowerCase()=='the-world-revolving' || FlxG.save.data.moveNotes){
                        if(daNote.mustPress){
							daNote.angle = 300*Math.cos((timer*2)+daNote.ID*2);
                            daNote.y = daNote.y+(-50*Math.sin((timer*2)+daNote.noteData*2));
                            daNote.x = daNote.health+(-160*Math.cos(((timer*4)+daNote.noteData/2)));
                        }
                    }
					if (FlxG.save.data.downscroll)
						daNote.y = (strumLine.y - (Conductor.songPosition - daNote.strumTime) * (-0.45 * FlxMath.roundDecimal(SONG.speed, 2)));
